(() => {
    ...
    const formBlock = document.querySelector(".sidebar");
    document.addEventListener("click", formWatch);
    function formWatch(event) {
        if (event.target.closest(".preview__btn")) formBlock.classList.toggle("active");
        if (event.target.closest(".menu__btn-close")) formBlock.classList.toggle("active");
    }
    window["FLS"] = true;
    isWebp();
    menuInit();
